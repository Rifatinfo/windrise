...
export function ReviewSummaryPanel({ summary }: { summary: Summary }) {
  ...
  return (
    <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:gap-6 lg:gap-9">
      {/* Average */}
      <div className="flex shrink-0 flex-col items-center gap-1 sm:pr-6 sm:border-r sm:border-[#e9e9e9] lg:pr-9">
        <p className="flex items-baseline gap-1.5">
          <span className="text-[26px] font-semibold leading-none text-[#1a1a1a] lg:text-[30px]">
            {summary.average?.toFixed(1)}
          </span>
          <span className="text-[11px] text-[#8f8f8f] lg:text-[12px]">out of 5</span>
        </p>
        <Stars value={summary.average ?? 0} size={14} />
        <p className="text-[11px] text-[#8f8f8f] lg:text-[12px]">
      ...
      <dl className="min-w-0 flex-1 space-y-[5px]">
        {summary.distribution.map((row) => (
          <div key={row.stars} className="flex items-center gap-2.5">
            <dt className="w-[42px] shrink-0 text-[10.5px] text-[#6f6f6f] lg:text-[11px]">
              {row.stars} Star
            </dt>
            <dd className="min-w-0 flex-1">
              <span
                className="block h-[5px] w-full overflow-hidden rounded-full bg-[#ececec]"
                role="meter"
